  ...
  new FacebookStrategy(
    ...
    async function (request, access_token, refresh_token, profile, done) {
      ...
      if (request.query.state) {
            ...
            if (
              ...
              if (token.type != "access") {
                ...
                db.user
                  ...
                  .then((user) => {
                    link_account(user, profile, "Facebook")
                      .then((linked_user) => {
        ...
        db.user
          .findOne({ "oauth.facebookoauthid": profile.id })
          .then((user) => {
            if (user) {
              // User found:
              ...
              register
                ...
                .catch((err) => {
                  if (err === "Email already exists") {
                    // If user account already exists, link it to their Facebook account
                    link_account_email(profile, "Facebook")
                      .then((linked_user) => {
                        return done(null, linked_user);
                      })
                      .catch((err) => {
